...
/// （正常+异常）：默认 `init()` 后 `GarrisonUtil::login` 可用、
...
/// （正常+异常）：`allow()` 声明的权限可命中，`deny_all()` 后全部不命中。
#[tokio::test]
#[serial]
...
/// （异常）：`fail_with` 注入的错误必须原样上抛，不得被降级为 `Ok(false)`。
#[tokio::test]
#[serial]
