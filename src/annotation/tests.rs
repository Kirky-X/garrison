...
use crate::stp::{with_current_token, GarrisonInterface, GarrisonUtil};
use axum::body::Body;
...
fn make_config(throw_on_not_login: bool) -> crate::config::GarrisonConfig {
...
/// 构建带 garrison_token header 的 axum Parts。
fn make_parts_with_garrison_header(token: &str) -> axum::http::request::Parts {
    let req = Request::builder()
        .method("GET")
        .uri("/protected")
...
// ----------------------------------------------------------------
// IntoResponse for GarrisonError 测试
// ----------------------------------------------------------------
...
fn expired_token_returns_401() {
    let err = GarrisonError::ExpiredToken("test".to_string());
    let response = err.into_response();
    assert_eq!(response.status(), StatusCode::UNAUTHORIZED);
}
...
fn internal_error_returns_500() {
    let err = GarrisonError::Internal("test".to_string());
    let response = err.into_response();
...
fn session_error_returns_500() {
    let err = GarrisonError::Session("test".to_string());
    let response = err.into_response();
    assert_eq!(response.status(), StatusCode::INTERNAL_SERVER_ERROR);
}
...
/// CheckLogin 从 garrison_token header 提取 token 并校验通过。
#[tokio::test]
#[serial]
async fn check_login_extracts_token_from_garrison_header() {
    ...
    assert!(
        result.is_ok(),
        "garrison_token header 提取 token 后校验应通过"
    );

    GarrisonManager::reset_for_test();
}

/// CheckLogin 从 Cookie: garrison_token=<token> 提取 token 并校验通过。
#[tokio::test]
#[serial]
...
// ----------------------------------------------------------------
// GarrisonPrincipal extractor 测试（携带 login_id）
// ----------------------------------------------------------------

/// `GarrisonPrincipal::from_request_parts` 从 `Authorization: Bearer <token>`
/// header 解析出 `login_id`。
///
/// 与 actix/warp extractor 对齐：valid token → Ok(GarrisonPrincipal { login_id })。
#[tokio::test]
#[serial]
async fn garrison_principal_extracts_login_id_from_bearer_header() {
    ...
    let mut parts = make_parts_with_bearer(&token);
    let principal = crate::context::GarrisonPrincipal::from_request_parts(&mut parts, &())
        .await
...
/// `GarrisonPrincipal::from_request_parts` 在无 token 时返回 `Err(NotLogin)`。
///
...
/// `GarrisonPrincipal::from_request_parts` 在无效 token 时返回 `Err(NotLogin)`。
///
...
/// `GarrisonPrincipal::from_request_parts` 在 token 曾有效但已 logout 时
/// 返回 `Err(NotLogin)`。
///
