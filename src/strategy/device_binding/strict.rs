...
//! - `require_secondary_auth`：新设备返回 `Ok(true)`（强制二级认证），旧设备返回 `Ok(false)`
//!
...
impl DeviceBindingPolicy for StrictBinding {
    ...
    async fn require_secondary_auth(&self, login_id: &str, device_id: &str) -> BulwarkResult<bool> {
        // 新设备强制二级认证；旧设备直接放行
        self.is_new_device(login_id, device_id).await
    }
}
...
mod tests {
    ...
    /// 旧设备时 require_secondary_auth 返回 false（直接放行）。
    #[tokio::test]
    async fn require_secondary_auth_returns_false_for_known_device() {
        let (_dao, session) = make_session();
        create_session_with_device(&session, "1001", "T1", "web-chrome").await;
        ...
        assert!(
            !require,
            "已知设备应直接放行（require_secondary_auth 返回 false）"
        );
    }
