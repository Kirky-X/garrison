...
impl UserRoleRepository for DbnexusUserRoleRepository {
    async fn find_by_user_id(
        ...
        let rows = conn.query_all_raw(stmt).await.map_err(|e| {
            GarrisonError::Dao(format!("dao-app-user-role-find-by-user-id-query::{}", e))
        })?;
    ...
    async fn find_by_role_id(
        ...
        let rows = conn.query_all_raw(stmt).await.map_err(|e| {
            GarrisonError::Dao(format!("dao-app-user-role-find-by-role-id-query::{}", e))
        })?;
        rows.iter().map(parse_user_role_row).collect()
    }
    ...
    async fn assign(
        ...
        conn.execute_raw(stmt)
            .await
            .map_err(|e| GarrisonError::Dao(format!("dao-app-user-role-assign-insert::{}", e)))?;
        Ok(())
    }
    ...
    async fn revoke(&self, tenant_id: i64, user_id: &str, role_id: &str) -> GarrisonResult<()> {
        ...
        conn.execute_raw(stmt)
            .await
            .map_err(|e| GarrisonError::Dao(format!("dao-app-user-role-revoke-delete::{}", e)))?;
        Ok(())
    }
    ...
    async fn list(
        ...
        let rows = conn
            .query_all_raw(stmt)
            .await
            .map_err(|e| GarrisonError::Dao(format!("dao-app-user-role-list-query::{}", e)))?;
        rows.iter().map(parse_user_role_row).collect()
    }
}

/// 解析 app_user_role 行。
fn parse_user_role_row(row: &QueryResult) -> GarrisonResult<UserRoleRow> {
    Ok(UserRoleRow {
        user_id: row.try_get("", "user_id").map_err(|e| {
            GarrisonError::Dao(format!("dao-app-user-role-row-parse-user-id::{}", e))
        })?,
        role_id: row.try_get("", "role_id").map_err(|e| {
            GarrisonError::Dao(format!("dao-app-user-role-row-parse-role-id::{}", e))
        })?,
        scope: row
            .try_get("", "scope")
            .map_err(|e| GarrisonError::Dao(format!("dao-app-user-role-row-parse-scope::{}", e)))?,
        grant_time: row.try_get("", "grant_time").map_err(|e| {
            GarrisonError::Dao(format!("dao-app-user-role-row-parse-grant-time::{}", e))
        })?,
        tenant_id: row.try_get("", "tenant_id").map_err(|e| {
            GarrisonError::Dao(format!("dao-app-user-role-row-parse-tenant-id::{}", e))
        })?,
    })
}
