...
mod embedded_migrations_tests {
    ...
    /// 验证 POSTGRES_MIGRATIONS 嵌入了 10 个 postgres core SQL 文件。
    ///
    /// Scenario: 编译时 include_dir!("migrations/postgres") 嵌入成功。
    /// WHEN POSTGRES_MIGRATIONS.get_dir("core")
    /// THEN core 目录存在且包含 10 个 .sql 文件（001_init ~ 010_audit_logs_composite_index）
    #[test]
    fn embedded_postgres_migrations_contain_10_core_files() {
        ...
        assert_eq!(
            sql_files.len(),
            10,
            "postgres core 迁移必须有 10 个 SQL 文件，实际: {sql_files:?}"
        );
        // 验证文件名边界（按版本号约定）
    ...
    fn copy_embedded_dir_writes_files_to_tempdir() {
        ...
        let entries: Vec<_> = std::fs::read_dir(&core_dir)
            ...
            .collect();
        assert_eq!(entries.len(), 10, "core 目录必须包含 10 个 SQL 文件");
    ...
    /// THEN 第二次成功，core 目录仍为 10 个文件（覆盖写入）
    #[test]
    fn copy_embedded_dir_is_idempotent() {
        ...
        let count = std::fs::read_dir(&core_dir)
            .expect("读取 core 目录应成功")
            .count();
        assert_eq!(count, 10, "重复写入后仍应为 10 个文件");
    }
