...
/// 构建 GarrisonRouter（含 CheckLogin 路由 + CheckRole + CheckPermission）。
fn make_router() -> GarrisonRouter {
...
// ----------------------------------------------------------------
// DefaultGarrisonInterceptor.pre_handle 测试
// ----------------------------------------------------------------

/// DefaultGarrisonInterceptor.pre_handle(CheckLogin) 已登录返回 Ok。
#[tokio::test]
#[serial]
...
/// DefaultGarrisonInterceptor.pre_handle(CheckLogin) 未登录返回 NotLogin。
#[tokio::test]
#[serial]
...
/// DefaultGarrisonInterceptor.pre_handle(CheckRole) 持有角色返回 Ok。
#[tokio::test]
#[serial]
...
/// DefaultGarrisonInterceptor.pre_handle(CheckPermission) 未持有权限返回 NotPermission。
#[tokio::test]
#[serial]
...
/// DefaultGarrisonInterceptor.pre_handle(Ignore) 直接返回 Ok。
#[tokio::test]
#[serial]
...
// ----------------------------------------------------------------
// DefaultGarrisonInterceptor 其他注解变体测试（catch-all 分支）
// ----------------------------------------------------------------

/// DefaultGarrisonInterceptor.pre_handle 对逻辑组合注解（CheckOr / CheckAnd / CheckNot）
...
/// DefaultGarrisonInterceptor.pre_handle(CheckSafe) 默认行为随 `safe-auth` feature 变化。
///
...
/// DefaultGarrisonInterceptor.pre_handle(CheckDisable) 默认实现返回 Ok（未禁用）。
#[tokio::test]
#[serial]
...
/// DefaultGarrisonInterceptor.pre_handle(CheckBasicAuth) 返回 NotImplemented（需 HTTP 请求上下文）。
#[tokio::test]
#[serial]
...
/// DefaultGarrisonInterceptor.pre_handle(CheckDigestAuth) 返回 NotImplemented（需 HTTP 请求上下文）。
#[tokio::test]
#[serial]
...
/// DefaultGarrisonInterceptor.pre_handle(CheckSign) 返回 NotImplemented（需 HTTP 请求上下文）。
#[tokio::test]
#[serial]
...
async fn default_interceptor_check_basic_auth_error_message_contains_guidance() {
    ...
    let result = interceptor
        .pre_handle("/x", &Annotation::CheckBasicAuth)
        .await;
    if let Err(GarrisonError::NotImplemented(msg)) = result {
        assert!(
...
/// DefaultGarrisonInterceptor.pre_handle(CheckAccessToken) 返回 NotImplemented
///（无 OAuth2Handler 注册）。
#[tokio::test]
#[serial]
...
/// DefaultGarrisonInterceptor.pre_handle(CheckClientToken) 返回 NotImplemented
///（无 OAuth2Handler 注册）。
#[tokio::test]
#[serial]
...
// ----------------------------------------------------------------
// GarrisonRouter::with_interceptor / Default 测试
// ----------------------------------------------------------------
...
#[async_trait]
impl GarrisonInterceptor for CountingInterceptor {
...
/// 验证 `GarrisonRouter::default()` 使用 `GarrisonConfig::default_config()`
/// 创建路由器，拦截器为 `DefaultGarrisonInterceptor`。
///
/// 覆盖 `impl Default for GarrisonRouter` 的 `default()` 方法。
#[tokio::test]
#[serial]
...
#[async_trait]
impl GarrisonInterceptor for RenewingInterceptor {
    async fn pre_handle(&self, _path: &str, _annotation: &Annotation) -> GarrisonResult<()> {
...
async fn renewed_token_written_to_cookie() {
    ...
    assert!(
        cookie_str.contains("garrison_token=renewed-cookie-token"),
...
async fn renewed_token_not_written_when_both_disabled() {
    ...
    assert_eq!(response.status(), StatusCode::OK);
    assert!(
        response.headers().get("garrison_token").is_none(),
...
async fn renewed_token_written_to_both_header_and_cookie() {
    ...
    assert!(
        cookie
            .to_str()
            .unwrap()
            .contains("garrison_token=dual-token"),
...
async fn no_renewed_token_nothing_written() {
    ...
    // 使用 DefaultGarrisonInterceptor（Annotation::Ignore 为 no-op，不触发续签）
    let app = GarrisonRouter::new(Arc::new(config))
        .route_protected("/test", || async { "ok" }, Annotation::Ignore)
        .build();
...
async fn clear_renewed_token_prevents_leak() {
    ...
    // 第一次请求：触发续签
    let app1 = GarrisonRouter::new(Arc::new(config.clone()))
        .with_interceptor(RenewingInterceptor {
    ...
    assert!(
        resp1.headers().get("garrison_token").is_some(),
        "第一次请求应有续签 header"
    );

    // 第二次请求：不触发续签（DefaultGarrisonInterceptor + Ignore = no-op）
    let app2 = GarrisonRouter::new(Arc::new(config))
        .route_protected("/test", || async { "ok" }, Annotation::Ignore)
        .build();
...
async fn cookie_has_correct_attributes() {
    ...
    assert!(
        cookie_str.contains("garrison_token=attr-check-token"),
