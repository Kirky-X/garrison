...
//! [`UserLockoutStrategy`](crate::account::lockout::UserLockoutStrategy) 直接通过
//! [`GarrisonDao`](crate::dao::GarrisonDao) 持久化 [`LockoutState`](crate::account::lockout::LockoutState)，
//! 未来版本可在此文件实现专用的锁定状态存储后端（如 Redis TTL 自动过期、
//! SQL 持久化、跨数据中心同步）。
//!
//! # 与 `UserLockoutStrategy` 的关系
//!
//! `UserLockoutStrategy` 当前使用 `GarrisonDao` 通用抽象
