...
# 1. 环境变量（可由调用方覆盖）
export EXAMPLE_INTERNAL_API_KEY="${EXAMPLE_INTERNAL_API_KEY:-e2e-test-key-12345}"
...
EXTERNAL_URL="http://127.0.0.1:${BULWARK_EXTERNAL_PORT}"
HEALTH_OK=false
for i in $(seq 1 30); do
    if curl --fail --silent --max-time 1 "${EXTERNAL_URL}/api/v1/auth/login" \
        -X POST \
        -H "Content-Type: application/json" \
        -H "X-Tenant-Id: 0" \
        -d '{"login_id":"health_probe","params":{}}' \
        >/dev/null 2>&1; then
