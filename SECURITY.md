# 🔒 Garrison 安全文档

Garrison 是一个面向 Rust 生态的身份认证鉴权框架，安全是项目的核心价值之一。本文档说明支持的版本范围、漏洞报告流程、响应承诺与安全配置建议。

> **协议与许可证**：项目以 Apache-2.0 协议开源。安全漏洞报告与披露遵循本文件描述的协调披露流程。
> 部署安全加固详见 [🚀 部署指南](docs/DEPLOYMENT.md)；配置项说明详见 [⚙️ 配置指南](docs/CONFIGURATION.md)。

## 📋 目录

- [支持的版本](#-支持的版本)
- [报告漏洞](#-报告漏洞)
- [响应承诺](#️-响应承诺)
- [披露策略](#-披露策略)
- [安全配置建议](#️-安全配置建议)
- [供应链与门禁](#️-供应链与门禁)
- [已知安全考量](#️-已知安全考量)
- [相关文档](#-报告漏洞)

---

## 📌 支持的版本

Garrison 仅对已发布版本提供安全修复，开发分支（`main`）不承诺向后移植补丁。

| 版本 | 支持状态 | 说明 |
|------|---------|------|
| 0.8.x | ✅ 维护中 | 当前稳定版本，接受安全修复 |
| 0.7.x | 🔶 有限维护 | 仅修复严重漏洞 |
| < 0.7 | ❌ 不支持 | 不提供安全修复 |

> 新版本发布后，旧版本将进入仅修复严重漏洞的「有限维护」状态，至少持续 6 个月。

---

## 🐛 报告漏洞

### 请勿公开提交 Issue

**严禁通过公开的 GitHub Issue、Pull Request、Discussions 或任何公开渠道披露安全漏洞。** 公开提交可能导致漏洞在被修复前被利用，危害所有用户。

### 报告渠道

请通过以下私密渠道报告：

1. **首选 — 邮箱**：发送邮件至 <Kirky-X@outlook.com>
2. **次选 — GitHub Security Advisory**：通过 [GitHub Security Advisory](https://github.com/Kirky-X/garrison/security/advisories/new) 提交（建议使用，支持加密通信与 CVE 申请）

### 报告内容要求

为便于快速评估与修复，请尽量在报告中包含：

- **漏洞描述**：清晰说明漏洞类型与本质（如越权访问、令牌伪造、会话固定等）。
- **受影响版本**：明确标注你测试时使用的 Garrison 版本与启用的 feature 组合（如 `--features "full"` 或 `--features "production"`）。
- **复现步骤**：
  - 最小可复现代码片段或测试用例。
  - 相关配置（`garrison.toml` 片段，注意隐去敏感信息如密钥）。
  - 运行环境（OS、Rust 版本、依赖版本）。
- **影响范围评估**：
  - 攻击者可达成什么（如越权读取、伪造身份、DoS）。
  - 触发条件（是否需要登录、是否需要特定配置）。
  - 预估受影响用户范围。
- **建议修复方案**（可选）：如果你已研究过修复方向，欢迎一并提出。

---

## ⏱️ 响应承诺

| 阶段 | 时间窗口 | 行动 |
|------|---------|------|
| 确认收到 | **48 小时内** | Maintainer 确认收到报告，并指派负责人 |
| 初步评估 | 7 天内 | 评估漏洞等级（Critical / High / Medium / Low）与影响范围 |
| 修复计划 | 14 天内 | 与报告者沟通修复方案与发布计划 |
| 修复发布 | 90 天内（依据严重程度调整） | Critical / High 优先紧急发布 patch 版本 |

修复发布后，将在 GitHub Security Advisories 中公开致谢报告者（如同意）。

---

## 📢 披露策略

Garrison 采用 **responsible disclosure（负责任披露）** 策略：

1. **保密期**：漏洞报告后至修复发布前，双方对漏洞细节保密。
2. **协调发布**：修复版本发布时，同步公开漏洞详情与 CVE（如适用）。
3. **致谢**：在安全公告中致谢报告者（如同意）。
4. **提前披露**：若报告者同意，可在修复发布前与受影响的下游关键用户共享漏洞信息以便提前准备。

> 若报告者希望公开披露，请与 Maintainer 协调发布时间，确保修复版本已上线后再公开。

---

## 🛡️ 安全配置建议

在生产环境部署 Garrison 时，以下配置项**必须修改**或特别关注：

### 1. JWT 密钥（必改）

- **必须使用强随机字符串**，长度 **≥ 32 字节**（256 位）。

  ```bash
  # 生成 32 字节随机密钥（base64 编码）
  openssl rand -base64 32
  ```

- 通过 `GARRISON_JWT_SECRET` 环境变量注入，**不得硬编码到代码或配置文件**。
- 禁止使用弱密钥（如 `secret`、`123456`、项目名等）。
- 禁止在多个环境间复用同一密钥（开发/测试/生产必须独立）。

### 2. Redis 连接（生产必改）

- 生产环境**必须启用 `cache-redis`** 而非 `cache-memory`。
  - `cache-memory` 基于 oxcache 内存缓存，进程重启即丢失，多实例部署时无法共享会话状态。
  - `cache-redis` 通过 oxcache 接入 Redis，支持分布式部署与会话共享。
- 通过 `GARRISON_REDIS_URL` 配置 Redis 连接，建议启用 TLS 与密码认证。

  ```env
  GARRISON_REDIS_URL=rediss://:password@redis-host:6379/0
  ```

### 3. 环境变量管理

- `GARRISON_JWT_SECRET` 环境变量 **不得写入版本库**。
  - ✅ 正确：通过环境变量注入、密钥管理服务（如 Vault、AWS Secrets Manager）或 `.env` 文件（且 `.env` 已被 `.gitignore` 排除）。
  - ❌ 错误：在 `garrison.toml` 或源码中明文写入 secret。
- 检查项：提交前确认 `git diff` 中无密钥泄露，CI 应配置 secret 扫描（如 git-secrets、trufflehog）。

### 4. TLS 终止

- **HTTPS 必须由反向代理终止**（如 Nginx、Caddy、AWS ALB），Garrison 本身不处理 TLS。
- 确保反向代理与 Garrison 之间为可信网络（同主机、内网或 Service Mesh mTLS）。
- 反向代理应强制 HSTS、禁用旧版 TLS（≤ 1.1）。

### 5. 密钥轮换

- **定期轮换 API Key 与 JWT 密钥**：
  - JWT 密钥建议每 90 天轮换一次，发现疑似泄露立即轮换。
  - 支持多密钥并存过渡：新密钥签发 + 旧密钥仅验证，过渡期 ≥ 7 天后下线旧密钥。
  - API Key 建议每 180 天轮换，长期未使用的 Key 应主动吊销。

### 6. 会话超时

- 合理配置 `timeout` 与 `active_timeout`，避免过长有效期的会话令牌。
- 高安全场景建议缩短 `active_timeout`（如 3600 秒 = 1 小时）。

### 7. 最小权限与审计

- 通过 RBAC 配置时，仅授予完成业务所需的最低权限，避免通配权限（如 `*`）。
- 启用 `listener` 与 `tracing-log` feature，记录认证/授权关键事件，便于事后追溯。

### 8. API Key 安全（`protocol-apikey`）

Garrison 的 API Key（`protocol-apikey`）采用以下安全设计：

- **凭证格式**：`key_id.key_secret` 双段（各 32 hex，`.` 分隔）。`key_id` 为公开标识，可安全记录到日志用于审计；`key_secret` 为机密部分，仅在生成时返回一次。
- **哈希存储（CWE-916）**：`key_secret` **永不落库**，仅存储其 `sha256` 哈希，校验时用常量时间比较（`subtle::ConstantTimeEq`）。KV/数据库泄露也无法还原 secret。
  - 说明：API Key 为高熵随机值（244-bit），采用确定性 SHA-256 而非 Argon2，以保持 O(1) 查询并避免每请求慢哈希开销；此选择的前提是生成期锁定的高熵不变量。
- **IP 级失败限速（CWE-307）**：同时启用 `firewall-bruteforce` 并在 Web middleware 通过 `with_current_ip` 注入客户端 IP 后，`check_api_key` 会在校验失败时按 IP 计入暴力破解防护（仅失败计数，成功零写入），超阈值封禁。IP 应使用 `extract_client_ip`（含 trusted_proxies 防 X-Forwarded-For 伪造）提取。
- **多租户隔离（IDOR）**：`verify_with_namespace` 强制校验 key 归属的 namespace，namespace A 的 key 无法在 namespace B 通过；启用 `tenant-isolation` 后 DAO 层按租户自动加 key 前缀，提供物理隔离。`ApiKeyInfo.owner_id` 记录归属主体（默认等于 `login_id`）供审计。
- **持久化**：API Key 存储于 DAO 后端。生产环境**必须启用 `cache-redis`** 而非 `cache-memory`（后者进程重启即丢失全部 key）；Redis 建议启用 AOF（`appendfsync everysec`），不要仅依赖 RDB 快照，以免崩溃丢失最近的生成/吊销操作。如需 ACID 与外键约束级持久化，参见 `docs/decisions/` 中的 DB-of-record 评估。
- **密钥管理**：支持 `last_used_at` 使用追踪（`with_last_used_tracking(true)` 启用，节流写入）、`get_keys_older_than` 陈旧 key 识别（轮换策略）、每 key `rate_limit`，以及 `with_allowed_scopes` 生成期 scope 白名单校验。
- **向后兼容**：旧格式单 token（v0.4.1/v0.4.2，无 `secret_hash`）仍可校验（按存在性），随其 TTL 自然过期淘汰，建议尽快轮换为新双段格式。

---

## ⛓️ 供应链与门禁

Garrison 在 CI 与发布流程中内置多层供应链与安全门禁（发布阶段门禁标准见 [📦 发布工作流](./docs/RELEASING.md)）：

**依赖与发布门禁**：

- **`cargo deny check`**（`deny.toml`）：安全公告、许可证、禁用依赖与来源校验，CI 每次 PR 执行（`ci.yml` deny 步骤），发布门禁要求 0 failures。
- **`cargo audit`**：RustSec 安全公告扫描，发布门禁要求 0 CRITICAL；`deny.toml` 中的忽略项均注明理由（如 RUSTSEC-2023-0071，见下文[已知安全考量](#️-已知安全考量)）。
- **`cargo vet`**（`supply-chain/`）：供应商依赖人工审核记录——自研库（CodeNexus 系）已建立 `safe-to-deploy` 审核条目，第三方依赖豁免清单渐进收紧；审核记录公开可复核（众包审计背书）。
- **SBOM（CycloneDX）**：`security.yml` 全特性面生成 `garrison.cdx.json`（覆盖 optional 依赖），并作为 Release 资产附加（`release.yml`），下游可审计依赖构成。
- **Artifact Attestations**：Release 资产生成 SLSA 构建溯源（`actions/attest-build-provenance`），可用 `gh attestation verify` 验证产物来自本仓库 CI。

**静态扫描与模糊测试**（`security.yml`，PR 阻断）：

- **Semgrep SAST**：`p/rust` + `p/secrets` registry 规则集，覆盖认证路径反模式（unsafe 滥用、硬编码密钥）；忽略清单 `.semgrepignore` 排除测试/示例假凭证（对齐 codeql-config.yml 先例）。
- **gitleaks 全量 history 扫描**：CI 每次 PR 扫描全部 git 历史（pre-commit 仅覆盖 staged），`.gitleaksignore` 收录已逐条核实的假凭证指纹。
- **cargo-fuzz 模糊测试**：4 个解析入口 target（SAML XML / OIDC discovery / JWT verify / HTTP 认证头），PR 语料回归冒烟 + 每周一深扫（`fuzz-weekly.yml`），崩溃即阻断。

**基础卫生**：pre-commit 私钥扫描（gitleaks + `detect-private-key` 钩子互补）与 GitHub Actions 第三方 action 统一按 40 字符 commit SHA / 镜像 digest pin 引用（`ci.yml` / `security.yml` / `docs.yml` / `codeql.yml`），防 tag 替换类供应链攻击。

威胁模型与攻防边界详见 [🛡️ 威胁模型](./docs/THREAT.md)；OWASP Top 10 逐类映射见 [🗺️ OWASP Top 10](./docs/OWASP_TOP10.md)；数据合规与删除权见 [📋 数据合规指南](./docs/DATA_COMPLIANCE.md)；密码学选型论证见 [ADR](./docs/adr/0001-argon2id-password-hashing.md)。

---

## ⚠️ 已知安全考量

以下为 Garrison 当前已知的安全相关考量点，使用时请评估是否影响你的业务场景。

### 1. HTTP Digest MD5 算法已不安全

- **说明**：HTTP Digest 认证若使用 MD5 哈希算法，MD5 已被证明存在碰撞攻击，不再具备密码学安全性。
- **影响**：`protocol-httpdigest` 模块若显式指定 MD5 算法，可能受到碰撞攻击或离线字典攻击。
- **建议**：使用 **SHA-256** 替代 MD5。`DigestAlgorithm` 枚举已支持 `Sha256` 变体，通过 `HttpDigestAuth::new(realm, "SHA256")` 显式指定。
- **状态**：0.2.0 起已支持 SHA-256 算法（`DigestAlgorithm::Sha256`）。**自 0.7.0 起 `DigestAlgorithm::default()` 已切换为 `Sha256`**（safe-by-default）；仅当需要兼容旧客户端时，才通过 `HttpDigestAuth::new(realm, "MD5")` 显式指定 MD5。

### 2. API 签名 nonce 防重放窗口

- **说明**：`protocol-sign` 模块的 API 签名机制通过 nonce + 时间戳防重放，默认时间窗口为 **300 秒**（`sign_window_seconds`）。
- **影响**：在时间窗口内，攻击者若截获合法签名请求，可重放该请求。
- **建议**：
  - 高 QPS 场景：适当缩短时间窗口（如 60 秒）。
  - 关键写操作：在业务层补充幂等性控制，不完全依赖 nonce 防重放。
  - nonce 缓存需具备跨实例共享能力（使用 `cache-redis` 而非 `cache-memory`）。

### 3. is_share=true 共享会话风险

- **说明**：当 `is_share=true` 时，同一账号在多端共享同一会话令牌。
- **影响**：攻击者一旦在任一端窃取会话令牌，即可在所有端冒充该用户。
- **建议**：高风险场景（金融、支付）禁用 `is_share`，使用 per-device 独立会话。

### 4. remember_me 扩展会话超时

- **说明**：`remember_me_enabled = true` 时，登录请求 params 含 `remember_me=true` 将使用 `remember_me_timeout`（默认 7776000 秒 = 90 天）作为会话 TTL，远大于常规 `timeout`（默认 2592000 秒 = 30 天）。
- **影响**：长期有效的会话令牌增加了被窃取后利用的窗口期。
- **建议**：
  - 仅在受信任设备上启用 `remember_me`，公共/共享设备不应使用。
  - 根据业务风险调整 `remember_me_timeout`，高安全场景可缩短至 7 天（604800 秒）或禁用该功能。

### 5. RUSTSEC-2023-0071（rsa crate Marvin Attack）处置说明

- **说明**：依赖图中的 `rsa` crate 存在 Marvin Attack 定时侧信道通告（RUSTSEC-2023-0071），且当前**无已修复版本**。
- **影响评估**：Garrison 中 `rsa` 的全部使用路径均为**私钥签名或公钥验签**，不存在 RSA 解密路径，而 Marvin Attack 是针对 RSA **解密**操作的 padding oracle 时序侧信道，对签名/验签路径不可达：
  - `social-alipay`：RSA 签名（`SigningKey`/`Signer`，支付宝报文签名）。
  - `protocol-jwt` 非对称扩展（RS256/RS384/RS512）：私钥签名 + 公钥验签（`JwtHandler::with_rsa_private_pem`，验证端经 pkcs1/pkcs8 纯 DER 解析提取 `(n,e)` 构造 `DecodingKey`，garrison 代码不触碰 rsa crate 的解密 API）；JWKS 端点仅导出公钥成分（`n`/`e`），不新增攻击面。
  - config 层强制 RSA 私钥位长 ≥2048（`validate_core`），杜绝弱密钥配置。
- **处置**：`deny.toml` 显式忽略该通告并注明理由（无 patched 版本 + 签名/验签路径不受影响）；`rsa` 亦作为 dev-dependency 用于 `keycloak-oidc` 测试生成 RSA 密钥对（测试环境）。
- **回归锚定**：若未来引入 RSA **解密**能力（如 JWE 解密），必须重新评估该通告并补充防护（常数时间解密或等待 rsa crate 发布修复版本后升级并移除忽略项）。

---

如对本文档有任何疑问或建议，请通过 [GitHub Discussions](https://github.com/Kirky-X/garrison/discussions) 提出（非漏洞类）；安全漏洞请遵循 [报告漏洞](#-报告漏洞) 流程私密提交。
