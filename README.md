...
- **GarrisonManager** 全局单例，持有 `Arc<dyn GarrisonLogic>`，业务方启动时一次性注入依赖即可使用静态 API
...
| 路由分组 group() | ✅ 0.6.0 完成 | `GarrisonRouter::group(prefix, annotation, f)` |
...
graph TD
    ...
    Manager --> Logic["GarrisonLogic trait"]
...
- `garrison-stp`：核心 API（`GarrisonLogic` trait + `GarrisonUtil` 静态委托 + task_local 上下文）
...
| 特性 | 默认 | 说明 |
| --- | :---: | --- |
| `backend-embedded` | ✅ | 内嵌后端模式（进程内认证，委托 GarrisonManager，0.7.0 新增） |
...
Garrison 提供三层测试体系：单元测试（3776+ 个 lib）+ E2E 测试（65+ 个，含 API 矩阵 / 性能基线 / 渗透测试）+ doc-tests。
