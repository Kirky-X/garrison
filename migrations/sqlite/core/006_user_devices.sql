-- Migration: 用户设备表（v0.5.1 新增，依据 design.md D4 / M2 UserDevice）
...
--
-- 注意：时间字段用 INTEGER（epoch seconds），与 design.md D4 schema 一致，
...
CREATE TABLE IF NOT EXISTS app_user_device (
    ...
    tenant_id         INTEGER NOT NULL,                                    -- 租户 ID（i64）
    login_id          INTEGER NOT NULL,                                    -- 登录 ID（i64）
    device_identifier TEXT    NOT NULL,                                    -- UA hash 或设备指纹
    device_name       TEXT,                                                -- 从 UA 解析的设备名
    user_agent        TEXT,                                                -- 原始 User-Agent
    is_blocked        INTEGER NOT NULL DEFAULT 0,                         -- 0=未阻断, 1=已阻断
    last_seen_at      INTEGER,                                             -- 最后活跃时间（epoch seconds，可空）
    created_at        INTEGER NOT NULL,                                    -- 创建时间（epoch seconds）
