...
CREATE TABLE IF NOT EXISTS refresh_tokens (
    token_hash         VARCHAR(64) PRIMARY KEY,            -- SHA-256(token)，主键（hex 编码 64 字符）
    parent_token_hash  VARCHAR(64),                         -- 旧 token 的 SHA-256（首次签发为 NULL）
    login_id           VARCHAR(36) NOT NULL,                 -- 关联用户 ID（String）
