...
Bulwark 主要作为库（crate）集成到业务方的 axum / actix-web 服务中；自 v0.7.0 起也提供独立的 `auth_server` 二进制（双端口 axum 服务器，需 `auth-server` feature），可直接部署作为认证服务。下面以「库集成」为主流程，「`auth_server` 直接部署」见 1.3。

### 1.1 标准部署流程

1. **添加依赖**：在业务方 `Cargo.toml` 中添加 Bulwark 依赖并选择 feature。
2. **编写初始化代码**：在服务启动时调用 `BulwarkManager::init()`。
3. **执行数据库迁移**：通过 `BulwarkMigration::new(pool).run_all()` 自动建表。
4. **注册路由中间件**：通过 `BulwarkRouter::new(config).build()` 构建 axum Router（自动注入 `BulwarkLayer` middleware）。
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    // 1. 建立数据库连接（dbnexus）
    let db = init_dbnexus("sqlite:///data/bulwark.db?mode=rwc").await?;

    // 2. 执行迁移（幂等，首次启动自动建表）
    BulwarkMigration::new(db.clone()).run_all().await?;
    ...
    // 4. 初始化 BulwarkManager（同步函数，必须在所有 API 调用前）
    BulwarkManager::init(dao, config.clone(), interface)?;

    // 5. 启动 axum 服务（BulwarkRouter::build 自动应用 BulwarkLayer middleware）
...
若不想自建 axum 服务，可直接使用 Bulwark 自带的 `auth_server` 二进制（需 `auth-server` feature）：
...
# 启动（双端口 axum 服务器）
BULWARK_INTERNAL_API_KEY=$(openssl rand -base64 32) \
BULWARK_EXTERNAL_PORT=8080 \
BULWARK_INTERNAL_PORT=8081 \
BULWARK_RATE_LIMIT=100 \
...
- `BULWARK_INTERNAL_API_KEY` 必须配置（fail-closed，未设置时进程退出）
...
# 开发环境
[dependencies]
bulwark = { version = "0.7", features = ["development"] }
...
Bulwark 通过 `dbnexus` 的 auto-migrate 能力提供 `BulwarkMigration` 类型，启动时按 `core → extensions → tenant` 顺序执行迁移：
...
BulwarkMigration::new(pool).run_all().await?;  // 等价于 migrate_core + migrate_extensions + migrate_tenant
// 或仅执行核心表迁移：BulwarkMigration::new(pool).migrate_core().await?;
```
...
Bulwark 通过 `oxcache` 0.3（crates.io）提供两级缓存：
...
```toml
[dependencies]
bulwark = { version = "0.7", features = ["cache-memory"] }
```
...
```toml
[dependencies]
bulwark = { version = "0.7", features = ["cache-redis"] }
```
...
# 生产构建（利用 cargo 特性缓存层）
# 业务方自建二进制时替换 <your-binary>；直接用 Bulwark 自带二进制则改为 auth_server
...
Bulwark 作为库集成时不自带 HTTP 端点。**建议业务方**在集成时实现 `/health` 端点（若使用 `auth_server` 二进制，则自带健康检查端点）：
...
```toml
[dependencies]
bulwark = { version = "0.7", features = ["production", "protocol-jwt", "secure-totp"] }
```
