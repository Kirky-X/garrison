...
- `MockDao` 正名 `InMemoryDao`（`src/dao/in_memory.rs`；`deprecated` 别名过渡，已移除）
...
- **API Key 安全退化修复（nebulaid → garrison 迁移，详见 `docs/decisions/A-011`）**：
  ...
  - **legacy key fail-closed**（**破坏性变更**，W8/CWE-916 强化）：`decode_and_check` 拒绝空 `secret_hash` 的旧格式 key（v0.4.1/v0.4.2 单 token），返回 `InvalidToken("apikey-legacy-secret-required")` 并 `tracing::warn!` 迁移提示。**不提供 opt-in 兼容开关**（遵循"禁止向后兼容"规则），legacy key 必须迁移到 v0.7.x 双段格式。删除了 `with_allow_legacy_empty_secret` builder 方法。
- **tech-review 修复批次（specmark `fix-tech-review-findings`）**：
  - **W3 健康检查真实探测 + 超时**：`DbHealthCheck`/`CacheHealthCheck` 在网络后端（`db-postgres`/`db-mysql`/`cache-redis`）启用时执行真实 `dao.get(HEALTH_PROBE_KEY)` 探测，并以 `tokio::time::timeout(2s)` 包裹，超时/失败返回 `Unhealthy`（消除原"始终返回 Healthy"的假阳性）。嵌入式后端（`db-sqlite`/`cache-memory`）保持快路径 `Healthy`。`GarrisonSession::dao()` 访问器 feature gate 扩展到 `db-postgres`/`db-mysql`/`cache-redis` 供健康检查复用（`pub(crate)`）。
  - **W2' singleflight 锁清理**（CWE-770）：`three_tier.rs` 的 singleflight 加载路径在 drop write guard 后按 `Arc::strong_count(&lock) == 1` 从 `singleflight_locks` 移除对应 key，防止未绑定增长。
  - **W1 SQL 占位符转换**：`convert_placeholders` 引入 `in_string` 状态机 + `chars().peekable()` look-ahead，跳过 SQL 字符串字面量内的 `?`（仅替换引号外的 `?` 为 `$n`），正确处理 `''` 转义。
  - **W5 backend 测试外移**：`src/backend/mod.rs` 的内联 `#[cfg(test)] mod tests` 迁移到独立 `src/backend/tests.rs`，遵循 mod.rs 接口隔离规则。
  - **C1 PG/MySQL 补齐 007 OAuth2 字段迁移**：`migrations/postgres/core/007_refresh_tokens_oauth2_fields.sql` 和 `migrations/mysql/core/007_refresh_tokens_oauth2_fields.sql` 新增 `client_id`/`scopes`/`username`/`user_id` 字段及 `idx_refresh_client` 索引，与 SQLite 007 对齐。
  - **C2 PostgreSQL Repository re-export 对齐**：`src/dao/repository/postgres/mod.rs` 补齐 4 个别名（`AuthMethod`/`Session`/`LoginLog`/`UserExt`），与 mysql/mod.rs 完全对称（10 个 Repository）。
...
- 修复 commit 范围：`811ee10`..`171b7bd`（6 个 commit）
- 修复日期：2026-07-18
...
### strix garrison_e024 安全审查修复批次

**触发**：strix 渗透测试报告 `strix_runs/garrison_e024/` 中 9 个漏洞（4 CRITICAL + 3 HIGH + 2 MEDIUM）+ tiangang SAST 审查 3 MEDIUM + 7 LOW。
...
### strix garrison_0792 安全审查修复批次

**触发**：strix 渗透测试报告 `strix_runs/garrison_0792/` 中 3 个漏洞（1 HIGH + 2 MEDIUM）。
...
- `6e1ba9a` — **CRITICAL** 修复 sdforge `#[forge]` 路由注册失效：`#[forge]` 宏生成的 `inventory::submit!(RouteRegistration::new(...))` 被 `#[cfg(feature = "http")]` 保护，此 cfg 在 garrison 上下文求值（不是 sdforge）。garrison 缺少 `http` feature → submit 调用被 cfg 剥离 → `sdforge::http::build()` 收集不到任何路由 → 所有 29 个 sdforge_routes 测试 HTTP 404（production 代码也受影响）。修复：在 `[features]` 添加 `http = []` 桥接 feature（cfg flag only），让 `auth-server-sdforge = ["http"]`。sdforge/http 已由依赖声明无条件启用，此 feature 仅作 garrison 侧的 cfg 开关。
...
- 错误类型统一：全代码库使用 `GarrisonError` / `GarrisonResult`，0 违规
...
- `backend-embedded` feature：内嵌后端模式（默认启用）
- `backend-kit` feature：trait-kit typestate DI 构建
- `src/server/server_impl.rs` + `src/server/config.rs` + `src/server/middleware.rs` + `src/server/oauth2_routes.rs` + `src/server/sdforge_routes.rs`：认证服务器路由与中间件
- `src/bin/auth_server.rs`：独立认证服务器二进制
- `GarrisonAuthServer::with_tenant_resolver` builder：注入 `TenantResolver`，启用 `tenant_resolution_middleware`（`tenant-isolation` feature 门控）
...
- `GarrisonPermissionStrategyDefault` 在 `check_permission` / `check_role` 无记录时返回 `Deny` 而非 `Allow`
...
- `GarrisonConfig` 新增 `l1_cache_ttl_secs`（默认 30）/ `l2_cache_ttl_secs`（默认 300）/ `l1_cache_capacity`（默认 10000）
- `GarrisonLogicDefault` 新增 `user_cache_service` 字段 + `with_user_cache_service` builder
- `logout` / `logout_by_login_id` 在 `three-tier-cache` feature 启用时调用 `invalidate` 失效用户缓存
- `GarrisonManager::init` 自动构造 `UserCacheService` 并注入
...
- `GarrisonEvent::AnomalousLoginDetected` 事件变体 + audit.rs 记录
...
- `GarrisonConfig` 新增 `replaced_login_exit_mode`（默认 `OldDevice`）+ `overflow_logout_mode`（默认 `Logout`）
- 环境变量覆盖 `GARRISON_REPLACED_LOGIN_EXIT_MODE` / `GARRISON_OVERFLOW_LOGOUT_MODE`

#### D2: 从请求体读取 Token

- `GarrisonConfig` 新增 `is_read_body: bool` 字段（默认 false，向后兼容）
- 环境变量覆盖 `GARRISON_IS_READ_BODY`
...
- `GarrisonConfig` 新增 `login_token_map_persist_interval_secs: u64`（默认 0=同步写入）
...
- `GarrisonConfig` 新增 `device_binding_mode`（默认 "disabled"）
- 24 个单元测试
...
- `GarrisonManager` 注册 `DisableRepository`，`disable_repository()` 方法获取实例
- 30 个单元测试
...
- `GarrisonSession::cleanup_expired_tokens()`：扫描 login_token_map，移除过期/已注销 token
...
- 新建 `src/web/waf.rs` 模块，定义 `WafRule` trait（`async fn check(&self, ctx: &WafContext) -> GarrisonResult<()>`）+ `WafContext` struct
- 5 个内置规则：`DangerousCharacter`（检测 `//`/`\`/`%2e`/`%2f`/`;`/`\0`/`\n`/`\r`）、`DirectoryTraversal`（`./`/`../`/`..%2f`/`..%5c`）、`PathWhitelist`、`PathBlacklist`、`HttpMethodWhitelist`
- `garrison_waf_middleware` axum 中间件，遍历规则链，任一拒绝返回 400
- `GarrisonConfig` 新增 `waf_config: WafConfig`（enabled/path_whitelist/path_blacklist/check_dangerous_chars/check_directory_traversal/allowed_methods）
- 33 个单元测试
...
- `garrison_cors_middleware`：preflight（OPTIONS）请求注入 `Access-Control-Allow-*` 头返回 204；实际请求注入 `Access-Control-Allow-Origin/Expose-Headers`
- `CorsConfig::validate()`：`allow_credentials=true` 且 `allowed_origins` 含 `*` 时返回 Err
- `GarrisonConfig` 新增 `cors_config: CorsConfig`
- 29 个单元测试
...
- `garrison_csrf_middleware`：安全方法（GET/HEAD/OPTIONS）懒生成 token 设置 Cookie；保护方法（POST/PUT/PATCH/DELETE）校验 header==cookie，不一致返回 403
- `CsrfConfig`：enabled/cookie_name/header_name/excluded_paths/protected_methods/cookie_secure
- `GarrisonConfig` 新增 `csrf_config: CsrfConfig`
- 25 个单元测试

#### D4: 响应 Token 自动写入

- `src/stp/context.rs` 新增 `get_renewed_token() -> Option<String>` + `clear_renewed_token()`
- `GarrisonConfig` 新增 `is_write_cookie: bool`（默认 false，`is_write_header` 已存在）
- `garrison_middleware` 修复：handler 执行包裹 `with_renewed_token_scope`（v0.6.3 遗漏，导致续签 task_local 无效）
...
- `GarrisonConfig` 新增 `rate_limit_backend: RateLimitBackend`（默认 Memory）
- 30 个单元测试
...
- `GarrisonConfig` 新增 `auto_renewal_threshold: i64`（默认 -1 不启用，范围 -1 或 0-100）
- `GarrisonLogicDefault::check_and_renew` 在 `check_login` 路径中检查剩余 TTL 百分比，低于阈值时自动续签
- 非 JWT 模式调用 `renew_to_equivalent`，JWT 模式调用 `refresh_token`
- 续签结果通过 `CURRENT_RENEWED_TOKEN` task_local 传递给 Web 框架 middleware
- `GarrisonJwtClaims` 新增 `jti` (RFC 7519 §4.1.7) UUID 声明，保证同一秒内签发的 token 唯一
- 环境变量 `GARRISON_AUTO_RENEWAL_THRESHOLD` 覆盖支持

#### D2: 并发登录控制（`is_concurrent` / `is_share` / `max_login_count`）

- `GarrisonConfig` 新增 `is_concurrent: bool`（默认 true）/ `is_share: bool`（默认 false）/ `max_login_count: u32`（默认 0 不限制）
...
- `GarrisonSession` 新增 `login_token_map: DashMap<String, Vec<String>>` 内存索引，支持快速查询

#### D3: Refresh Token 轮换公共 API

- `SessionLogic` trait 新增 `refresh_access_token(&self, refresh_token: &str) -> GarrisonResult<(String, String)>` 方法
- `GarrisonLogicDefault` 实现委托 `RefreshTokenRotation::rotate`（需 `protocol-jwt` + `db-sqlite` feature）
- `with_refresh_token_rotation` builder 方法注入 `RefreshTokenRotation` 实例
- 未注入时返回 `GarrisonError::NotImplemented`
...
- `GarrisonUtil::login_simple(id)` 便捷方法用 `LoginParams::default()` 保持向后兼容
- 所有 mock impls 已同步更新
...
- **HIGH-001 修复**：`check_and_renew` 并发竞态致"会话假活"——新增 per-login_id `renewal_locks`（独立于 `GarrisonSession::login_locks`），续签前获取锁 + 二次检查 TTL；独立锁避免 `renew_to_equivalent` → `logout` → `login_locks` 死锁
...
- `GarrisonConfig` 新增 `session_hover_timeout`（默认 -1 不启用）
- `GarrisonSession` 新增 `last_active_time` + `check_hover_timeout` + `update_last_active`
...
- `GarrisonConfig` 新增 `frontend_separation`（默认 false）
- `validate()` 在启用时打印 info 日志提示 Token Header 模式
- 环境变量 `GARRISON_FRONTEND_SEPARATION` 覆盖支持
- 3 个单元测试
...
- `garrison-macros/Cargo.toml` version 0.5.0 → 0.5.1（sync fn 宏支持为功能新增）
...
Garrison 0.6.0 是"账号安全引擎版"，通过 specmark change `v0-6-0-account-security-engine` 实施 4 项核心能力（E-001/E-002/E-003/E-004）+ 4 项技术债清理（B-001/B-002/C-001/D-001）。引入 `account/` 模块作为账号安全能力中枢，提供 Credential SPI、密码策略引擎、用户锁定策略、AuthenticationFlow DSL，并补齐 i18n 社交登录异常消息与 Prometheus 指标。
...
- `DaoCredentialRepository` 基于 `GarrisonDao` 的凭证存储
...
#### E-003: UserLockoutStrategy + GarrisonFirewallStrategy

- `UserLockoutConfig` + `WaitStrategy` + `LockoutState`
- `UserLockoutStrategy`：基于用户的登录锁定策略
- `GarrisonFirewallStrategy`：整合 UserLockoutStrategy 到 GarrisonFirewall 框架
...
- `tests/integration/tenant_isolation.rs` 7 处修改：`login_id: i64` → `&str` 对齐 `GarrisonInterface` 签名
- 新增 `use garrison::{PermissionLogic, SessionLogic};`（trait 方法需导入 trait）
...
Garrison 0.5.3 是"功能补全版"，通过 specmark change `v0-5-3-feature-completion` 实施 4 项功能补全（A-015/A-014/A-012/A-013）。补齐 stp 模块拆分遗留、MySQL 后端、Firewall MaxMindDb 生产后端，并升级 oxcache。
...
- 6 个 `impl trait for GarrisonLogicDefault` 块移至对应子文件
...
Garrison 0.5.2 是"架构重构版"，通过 specmark change `v0-5-2-architecture-refactor` 实施 5 项架构重构（A-002/A-004/A-009/A-010/A-011）。这是 v1.0 API 冻结前的最后重构窗口，彻底清零架构债务。

> **破坏性变更（无向后兼容）**：原计划用 `#[deprecated]` 周期过渡，apply 阶段用户决策直接删除，彻底清零技术债。

### 变更

#### A-002: GarrisonLogic trait 拆分

- 21 方法上帝 trait `GarrisonLogic` 拆分为 6 个子 trait：
  - `GarrisonCore`（base，1 方法）
  - `SessionLogic: GarrisonCore`（10 方法）
  - `PermissionLogic: SessionLogic`（2 方法）
  - `TokenLogic: SessionLogic`（5 方法）
  - `MfaLogic: SessionLogic`（2 方法）
  - `PasswordLogic: SessionLogic`（1 方法）
- **直接删除 `GarrisonLogic`**（无 `#[deprecated]` 过渡，无 type alias）
- Manager 持有具体类型 `Arc<GarrisonLogicDefault>`（非 trait 对象）
...
- 移除 `GarrisonUtil::login_id_to_i64`（不再需要）
- `GarrisonUtil` 保留 `impl Into<String>` ergonomic 入口

#### A-009: oxcache `_sync` API 评估

- 评估结论：保留 `_sync` API（in-memory backend 下 <1μs vs `spawn_blocking` 10-50μs）
- 文档化 `GarrisonDaoOxcache` 性能约束（见 `docs/decisions/A-009-oxcache-sync-api-evaluation.md`）
- 后续跟进：引入 Redis/分布式 backend 时改用 async API

#### A-010: `keys()` 性能评估

- 评估结论：defer 到 oxcache 0.5+（`Cache.backend` 为 `pub(crate)`）
- 文档化 `GarrisonDao::keys()` 已知限制（见 `docs/decisions/A-010-dao-keys-performance-evaluation.md`）
- 业务方临时方案：自行维护 key 集合
...
- 每个子文件包含 trait 定义 + `GarrisonLogicDefault` impl 块 + 单元测试

### 破坏性变更

1. **`GarrisonLogic` trait 删除**：所有 `Arc<dyn GarrisonLogic>` 使用者必须迁移到 `Arc<GarrisonLogicDefault>` + 显式 `use` 子 trait
2. **`LoginId` newtype 删除**：所有 `LoginId` 使用者必须迁移到 `String`/`&str`
3. **Manager 返回类型变更**：`GarrisonManager::logic()` 返回 `Arc<GarrisonLogicDefault>`（非 `Arc<dyn GarrisonLogic>`）
4. **`get_login_id()` 返回类型变更**：`Option<i64>` → `Option<String>`
5. **`verify_token()` 返回类型变更**：`i64` → `String`
6. **`login_id` 参数类型变更**：`i64` → `&str`（所有 trait 方法 + `GarrisonInterface`）
...
Garrison 0.5.0 是"生产刚需版"，通过 specmark change
...
- **`audit-log` feature**：`AuditLogListener` 订阅 14 种 `GarrisonEvent`，
...
Garrison 0.4.2 是 0.4.x 系列的 MINOR 版本，通过 specmark change
`v0-4-2-gap-closure-and-features` 实施 origin 文档与代码实现对齐 gap 闭合 + 新功能增强。
共完成 16 个 Phase（capability），覆盖：LoginId newtype、GarrisonDao 扩展、密码哈希、
...
- **GarrisonDao 4 方法扩展**：新增 `set_permanent`（无 TTL）/`get_timeout`（查询剩余 TTL）/
  ...
  `GarrisonDaoOxcache` 重写 `set_permanent`/`get_timeout`/`rename` 使用 oxcache 原生 API
...
- `Cargo.toml` `[workspace].members` 新增 `"garrison-macros"`
...
- `GarrisonDao::keys` 默认实现返回 `NotImplemented`，`GarrisonDaoOxcache` 当前也返回
  `NotImplemented`（oxcache 0.3 不支持 key scan，待 v0.5.0+ 升级）
- `GarrisonDao::rename` 默认实现为 get→set_permanent→delete 三步，非原子；
  `GarrisonDaoOxcache` 重写为 get→ttl_sync→set_with_ttl_sync→delete 四步保留 TTL，
  但仍非原子（oxcache 0.3 无原子 rename API，待 v0.5.0+）
- `GarrisonEvent::TokenRevoke` 已在 `revoke_token` 调用时集成 broadcast；
...
- `LoginId::String` 形式在内部层（i64）尚未完成迁移，公开 API 接受 `impl Into<LoginId>`
  但 `login_id_to_i64` 对 `String` 形式返回 `GarrisonError::Config`（待 v0.5.0+ 完成内部层迁移）
- Strategy Registry 的 `DefaultFirewallStrategy` 为 no-op（`GarrisonLogic` 无
...
- `garrison-macros/Cargo.toml` + `garrison-macros/src/lib.rs` 新建（proc-macro crate
...
Garrison 0.4.0 聚焦于 0.2.0 协议层遗留 gap 的补齐。通过 openspec change
...
- **SSO Server 独立抽象（gap #5）**：新增 `protocol-sso-server` feature + `SsoServer`
  ...
  `IdentityCenterIdConverter` + `NoopSsoChannel`，支持通过共享 GarrisonDao 与 SsoClient
  间接通信
- **AloneCache 多 Redis 实例隔离（gap #6）**：新增 `alone-cache` feature + `AloneCache`
  装饰器（实现 GarrisonDao，入口拼接 key_prefix 后委托 inner dao）+ `AloneCacheManager`
...
Garrison 0.2.1 是 0.2.0 的 PATCH 版本，聚焦于：auto-wire gap 修复、协议层边界场景测试补全、
examples 工程化重组与文档补充。本版本不引入新协议或新功能特性，仅包含 bug 修复与稳定性优化。

### 修复

- **auto-wire gap（TG4+TG5）**：修复 `GarrisonManager::init` 未注入 PluginManager /
  ListenerManager / AuthLogic / PermissionChecker 的 gap。`GarrisonLogicDefault` 新增 4 个可选字段
  与 builder 方法，`login` / `logout` / `kickout` / `login_by_token` / `refresh_token` 现自动触发
  plugin 钩子与 listener 事件。`GarrisonLogicFactoryFn` 签名扩展为接收 `&GarrisonLogicFactoryContext`
...
- **auto-wire 集成测试（TG5.8）**：新增 3 个集成测试验证 `GarrisonUtil::login/logout` 自动触发
  plugin/listener
- **examples 工程化重组（TG2-TG3）**：examples 从零散 `.rs` 文件重组为 workspace member
  （`garrison-examples` crate，`publish = false`），每个 bin 配套独立 `tests/<name>.rs` 测试文件

### 变更

- `GarrisonLogicFactoryFn` 类型签名扩展（新增第 4 个参数 `&GarrisonLogicFactoryContext`）
  — 0.x.x 阶段可接受的不兼容变更，自定义 factory 需适配新签名
- `GarrisonLogicDefault` 新增 4 个字段（`plugin_manager` / `listener_manager` / `auth_logic` /
...
Garrison 0.2.0 在 0.1.0 核心基础设施上补全了 13 个占位特性域，覆盖全部能力。
...
- **protocol-sso**：SSO 单点登录 ticket 模型（签发、校验、销毁），64 字符随机 ticket，
  60 秒 TTL，一次性使用，`SsoClient` 跨子系统通过共享 `GarrisonDao` 实现 SSO
...
- **exception-system**：`GarrisonException` 异常类型体系（含 login_type、token_value 等上下文）
- **json-template**：`GarrisonJsonTemplate` / `GarrisonSerializer` trait（JSON 模板与序列化抽象）
- **plugin-system**：`GarrisonPlugin` trait + `GarrisonPluginManager` + inventory 编译期注册，
  生命周期钩子（on_login / on_logout / on_permission_check），插件失败仅 tracing::warn!
- **listener-system**：`GarrisonListener` trait + `GarrisonListenerManager` + 事件广播，
  6 个事件变体（Login / Logout / Kickout / PermissionDenied / RoleDenied / TokenExpired）

### 修改（3 个 capability）

- **core-auth-api**：扩展 `GarrisonLogic` trait，新增 `login_by_token` / `verify_token` / `refresh_token`
  默认方法（向后兼容 0.1.0）；修复 `generate_token` 对 "jwt" style 的支持（委托 `JwtHandler::sign`）
- **session-management**：扩展 `GarrisonSession`，支持 SSO ticket 关联与临时凭证关联
- **permission-role-check**：扩展 `GarrisonFirewallStrategyDefault`，集成 `PermissionChecker`，
...
- **auto-wire gap**：`GarrisonLogicDefault` 当前不持有 `GarrisonPluginManager` / `GarrisonListenerManager`，
  `GarrisonUtil::login` 不会自动触发 `on_login` / `Login` 事件。需用户在业务层手动调用
  plugin/listener manager。此 auto-wire 在延后任务 13.4/13.5 中实现
- **login_by_token 默认实现**：`GarrisonLogicDefault` 未 override `login_by_token`（返回 `NotImplemented`），
...
Garrison 0.1.0 是首个里程碑版本，实现了身份认证鉴权框架的核心基础设施。
...
- **错误类型体系**：`GarrisonError` 枚举，涵盖 12 个变体
  （NotLogin / NotPermission / NotRole / InvalidToken / ExpiredToken / Dao / Config / Internal / Session / Annotation / Context）
- **配置系统**：`GarrisonConfig` 全局配置，支持代码默认值 / toml 文件 / 环境变量三级配置源，
  通过 `tokio::sync::watch` 实现热更新
- **上下文抽象**：`GarrisonContext` / `GarrisonRequest` / `GarrisonResponse` / `GarrisonStorage` trait，
  解耦 Web 框架依赖；提供 axum 适配器 `AxumRequest` / `AxumResponse` / `AxumStorage` / `AxumContext`

#### 数据访问层

- **DAO 抽象**：`GarrisonDao` trait，提供 get / set / update / delete / expire 五元操作
- **oxcache 0.3 集成**：`GarrisonDaoOxcache` 实现，支持 per-entry TTL，
  通过 `Cache<K,V>::ttl()` 保留原有 TTL（依赖本地 oxcache 仓库）
- **dbnexus 0.2 集成**：`init_dbnexus` 初始化 + `GarrisonMigration` 8 张核心表迁移
  （users / oauth2_accounts / roles / permissions / user_roles / user_permissions / sessions / user_ext）

#### 会话与认证

- **双模会话管理**：`GarrisonSession` 支持 Account-Session（按 login_id 索引）+ Token-Session（按 token 索引）
- **核心认证 API**：`GarrisonLogic` trait 定义 login / logout / check_login / kickout 完整契约，
  `GarrisonLogicDefault` 默认实现
- **task_local 上下文**：`with_current_token` / `current_token` 提供 async 请求级 token 存取
- **权限校验策略**：`GarrisonFirewallStrategy` trait + `GarrisonFirewallStrategyDefault` 默认实现

#### 全局管理器

- **GarrisonManager 单例**：持有 `Arc<dyn GarrisonLogic>` 全局引用，支持显式 `init()` 初始化
  - 覆盖式更新 + `reset_for_test()`（cfg(test)）
- **inventory 编译期注册**：`GarrisonLogicFactoryEntry` 通过 `inventory::submit!` 注册工厂函数，
  支持编译期扩展
- **GarrisonUtil 静态委托**：8 个静态方法（login / logout / kickout / check_login / get_login_id /
  check_permission / check_role）委托到 `GarrisonManager::logic()?`

#### axum Web 框架集成

- **注解系统**：5 个 axum extractor（`CheckLogin` / `CheckRole<R>` / `CheckPermission<P>` /
  `Ignore` / `Mode<M>`），基于 Marker struct + 关联常量模式
- **`IntoResponse` 实现**：`GarrisonError` 自动映射 HTTP 状态码
  （401 Unauthorized / 403 Forbidden / 500 Internal Server Error）
- **GarrisonRouter**：包装 `axum::Router`，提供 `route_protected(path, handler, annotation)` 语法糖
- **GarrisonInterceptor**：`pre_handle` trait + `DefaultGarrisonInterceptor` 默认实现
...
- **示例代码**：
  ...
  - `examples/axum_integration.rs`：完整 Web 应用（GarrisonRouter + 4 路由 + 服务器启动，244 行）
...
- **GarrisonRouter::route_protected**：仅支持 GET 方法，其他方法待后续版本支持

### 最低支持 Rust 版本

- Rust 1.85+（部分 deps 如 inventory 0.3 要求 edition2024）

<!-- 注：mod-crate-hardening 加固内容已合入上方 [0.7.0] 主条目，无重复发布 -->
