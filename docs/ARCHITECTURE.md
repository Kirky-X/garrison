...
| `protocol/sso/saml` | `protocol-sso` | SAML 2.0 骨架：`SamlProvider` trait + `DefaultSamlProvider`（quick-xml 解析） | 0.6.0 |
| `protocol/sso/oidc` | `protocol-sso` | OIDC RP 骨架：`OidcProvider` trait + `DefaultOidcProvider`（discovery + token exchange） | 0.6.0 |
| `protocol/sso/channel` | `cache-redis` + `protocol-sso-server` | `RedisPubSubSsoChannel`：Redis pub/sub 跨实例 SsoChannel 实现 | 0.6.0 |
