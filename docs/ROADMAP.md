...
| 0.5.2 | ✅ 已完成 | 2026-07-08 | 架构重构版（BulwarkLogic trait 拆分 / LoginId newtype / oxcache _sync 评估 / keys 性能 / stp 模块拆分） |
...
| Unreleased | 🚧 进行中 | — | gap-closure-remaining（remember_me / Redis 部署模式 / switch_to / renew_to_equivalent / OAuth2 注解 / group() / SessionExpiryListener / SAML 2.0 / OIDC RP / Redis pub/sub SsoChannel） |
...
| # | 内容 | 核心变更 | 状态 | Commit |
|---|------|---------|------|--------|
| A-002 | BulwarkLogic trait 拆分 | 21 方法上帝 trait 拆分为 6 个子 trait（BulwarkCore/SessionLogic/PermissionLogic/TokenLogic/MfaLogic/PasswordLogic），**直接删除 BulwarkLogic**（无 deprecated 过渡） | ✅ 已完成 | `cbcedcb` `7c86a99` `bc63645` |
...
- Manager 持有具体类型 `Arc<BulwarkLogicDefault>`（非 trait 对象），调用方需显式 `use` 子 trait
...
| A-014 | stp/mod.rs 完整拆分 | 164KB `src/stp/mod.rs` 拆分为 11 个职责文件（新增 `tests.rs` + `interface.rs` + `util.rs`）；6 个 `impl trait for BulwarkLogicDefault` 块移至对应子文件；mod.rs 最终 12.4KB / 284 行 | ✅ 已完成 | `d5b945e` `5862e77` `57c9e59` `bc52803` |
...
| # | 内容 | 核心变更 | 状态 | Commit |
|---|------|---------|------|--------|
| E-001 | account/ 模块骨架 + Credential SPI | 新建 `account/` 顶层模块；`Credential` trait 统一凭证模型 SPI；`PasswordCredential` + `TotpCredential` 实现；`DaoCredentialRepository` 基于 `BulwarkDao`；迁移 `PasswordHasher` 到 `account/credential/password.rs`（删除 `secure-password` feature） | ✅ 已完成 | `003443a` `c99f03c` `0933a74` `1e5f3cb` `e395763` `ca67934` |
...
| E-003 | UserLockoutStrategy + BulwarkFirewallStrategy | `UserLockoutConfig` + `WaitStrategy` + `LockoutState`；`UserLockoutStrategy` 基于用户的登录锁定；`BulwarkFirewallStrategy` 整合到 BulwarkFirewall 框架；**破坏性**：`FirewallContext.login_id` `i64`→`String` | ✅ 已完成 | `644e78d` `09d1556` `527a19b` |
...
| B-002 | tenant_isolation 集成测试修复 | `login_id: i64` → `&str` 对齐 `BulwarkInterface` 签名（7 处修改） | ✅ 已完成 | `0eb8929` |
...
### Unreleased gap-closure-remaining（进行中）

通过 specmark change `gap-closure-remaining` 实施 11 项能力域补齐（T001-T011），补齐 Sa-Token 对标剩余 gap。
...
| T006 | 路由分组 group() | `BulwarkRouter::group()` 方法，支持路由分组与统一中间件应用 |
| T007 | 会话过期回调 | `SessionExpiryListener` trait + 异步回调机制 |
| T009 | SAML 2.0 骨架 | `SamlProvider` trait + `DefaultSamlProvider` 骨架实现 |
| T010 | OIDC RP 骨架 | `OidcProvider` trait + `DefaultOidcProvider` 骨架实现 |
| T011 | Redis pub/sub SsoChannel | `RedisPubSubSsoChannel` 实现 + `SsoChannel` / `SsoServer` trait 整合 |

**里程碑意义**：补齐 Sa-Token 对标剩余 gap，remember_me / 身份切换 / Token 置换 / OAuth2 注解 / 路由分组 / 会话过期回调 / SAML 2.0 / OIDC RP / Redis pub/sub SsoChannel 十一大能力就位。
