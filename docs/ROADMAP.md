...
| 0.6.2 | ✅ 已完成 | 2026-07-10 | 安全与性能增强：会话空闲超时 / XSS 防护 / 限流器 / 脱敏 / 缓存预热 |
| 0.6.3 | ✅ 已完成 | 2026-07-11 | Token 自动续期 / 并发登录控制 / 设备管理 |
| 0.6.4 | ✅ 已完成 | 2026-07-11 | WAF / CORS / CSRF / Redis 限流后端 |
| 0.6.5 | ✅ 已完成 | 2026-07-12 | 安全告警 / 设备绑定 / 封禁库 / 二级认证 |
| 0.6.6 | ✅ 已完成 | 2026-07-12 | 匿名会话 / 会话搜索 / login_token_map 持久化 |
...
- `src/server/server_impl.rs` + `src/server/config.rs` + `src/server/middleware.rs` + `src/server/oauth2_routes.rs` + `src/server/sdforge_routes.rs`：认证服务器实现
...
在 1.0 之前（0.x 阶段），minor 版本允许破坏性变更以完成技术债务清零。策略如下：

1. **优先使用 `#[deprecated]` 标记 + 文档警告**，至少经过一个 minor 版本过渡期再移除
2. **对技术债务清零场景，允许直接删除**（如 v0.5.2 删除 `GarrisonLogic` 上帝 trait 和 `LoginId` newtype），但必须：
   - 在 CHANGELOG 中明确标注破坏性变更
   - 提供迁移指南
   - 在 ROADMAP 中记录决策理由
3. 过渡期结束后在 major 版本中移除（1.0 后严格遵循 SemVer）

### 向后兼容承诺

- **v1.0.0 后**：严格遵循 SemVer，破坏性变更仅在 major 版本发布
- **0.x 阶段**：核心 API（`GarrisonManager` / `GarrisonUtil` / `GarrisonConfig`）尽量保持稳定；协议/安全层 API 可能在 minor 版本中调整，建议在生产环境中锁定具体 patch 版本
