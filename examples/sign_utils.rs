...
//!
//! 展示：
//! 1. HMAC-SHA256 签名（输出 64 字符 hex）
//! 2. HMAC-SHA512 签名（输出 128 字符 hex）
//! 3. Base64 编码与解码互逆
//!
//! 运行方式：
//! ```sh
//! cargo run --example sign_utils --features secure-sign
//! ```
...
fn main() -> BulwarkResult<()> {
    ...
    println!("    签名   = {}", sig_256);
    println!("    长度   = {} 字符（256 bit = 32 字节 → 64 hex）\n", sig_256.len());
    ...
    println!("    签名   = {}", sig_512);
    println!("    长度   = {} 字符（512 bit = 64 字节 → 128 hex）\n", sig_512.len());
