...
//!
//! 展示：
//! 1. BulwarkJsonTemplate 解析含 `${key}` 占位符的 JSON 模板
//! 2. render 递归替换嵌套对象/数组中的占位符
//! 3. BulwarkSerializerDefault 类型化 serialize / deserialize 往返
//!
//! 运行方式：
//! ```sh
//! cargo run --example json_template
//! ```
...
fn main() -> BulwarkResult<()> {
    ...
    let reparsed: serde_json::Value = serde_json::from_str(&rendered).expect("渲染结果应为合法 JSON");
