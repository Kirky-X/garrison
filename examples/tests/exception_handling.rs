...
//! 异常处理示例测试。
//!
//! 验证 run() 完整执行，以及 NotLoginException / GarrisonException 的构造与互转。

use garrison::error::GarrisonError;
use garrison::exception::{GarrisonException, NotLoginException};
use garrison_examples::infrastructure::exception_handling;
...
#[test]
fn test_garrison_error_to_exception_roundtrip() {
...
fn test_exception_to_error_conversion() {
    ...
    assert!(
        !err_str.is_empty(),
        "GarrisonException 转 GarrisonError 后 Display 不应为空"
    );
}
