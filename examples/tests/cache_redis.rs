...
//!
//! 验证 GarrisonDaoOxcache L1 DAO 操作 + Redis L2 配置：
//! - `GarrisonDaoOxcache::new()` 创建 L1 DAO
...
/// 测试 GarrisonDaoOxcache::new() 创建 L1 DAO（无需 Redis）。
#[tokio::test(flavor = "multi_thread")]
#[serial]
async fn test_create_l1_dao() {
    let dao = garrison_examples::infrastructure::cache_redis::create_l1_dao().await;
    // 不 panic 即通过
    drop(dao);
}
...
async fn test_demo_dao_operations() {
    let dao = garrison_examples::infrastructure::cache_redis::create_l1_dao().await;
    garrison_examples::infrastructure::cache_redis::demo_dao_operations(&dao)
        .await
...
fn test_build_redis_l2_config() {
    let builder = garrison_examples::infrastructure::cache_redis::build_redis_l2_config();
...
/// cargo test -p garrison-examples --test cache_redis --features cache-redis -- --ignored
/// ```
#[tokio::test(flavor = "multi_thread")]
#[serial]
#[ignore = "需要 Redis 实例运行，设置 OXCACHE_ALLOW_INSECURE_REDIS=I_UNDERSTAND_THE_RISKS"]
async fn test_create_tiered_cache_with_redis() {
    let cache = garrison_examples::infrastructure::cache_redis::create_tiered_cache()
        .await
