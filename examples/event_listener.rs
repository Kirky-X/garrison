...
//!
//! 展示：
//! 1. 实现 BulwarkListener trait 处理事件
//! 2. 通过 inventory::submit! 编译期注册监听器
//! 3. BulwarkListenerManager 收集并广播事件
//! 4. 单个监听器失败不中断广播
//!
//! 运行方式：
//! ```sh
//! cargo run --example event_listener --features listener
//! ```

// cargo run --example event_listener --features listener

use bulwark::error::BulwarkResult;
use bulwark::listener::{BulwarkEvent, BulwarkListener, BulwarkListenerEntry, BulwarkListenerManager};
    ...
    fn on_event(&self, event: &BulwarkEvent) -> BulwarkResult<()> {
        ...
        match event {
            BulwarkEvent::Login { login_id, token, device } => {
                println!("    [AuditListener] Login: login_id={}, token={}..., device={:?}",
                    login_id, &token[..8.min(token.len())], device);
            },
            BulwarkEvent::Logout { login_id, token } => {
                println!("    [AuditListener] Logout: login_id={}, token={}...",
                    login_id, &token[..8.min(token.len())]);
            },
            BulwarkEvent::PermissionDenied { login_id, permission } => {
                println!("    [AuditListener] PermissionDenied: login_id={}, permission={}",
                    login_id, permission);
            },
            BulwarkEvent::TokenExpired { token } => {
                println!("    [AuditListener] TokenExpired: token={}...",
...
fn main() -> BulwarkResult<()> {
    ...
    assert!(after > before);
    println!("    AuditListener 调用次数 +{}（FailingListener 失败未中断广播）\n", after - before);
    ...
    println!("    Clone 后匹配 Kickout: {}", matches!(cloned, BulwarkEvent::Kickout { .. }));
    println!();
