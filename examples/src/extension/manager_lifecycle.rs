...
//! Manager 生命周期示例：演示 GarrisonManager 初始化、inventory 工厂与 task_local 上下文。
//!
//! 流程：
...
//! 6. 多账号登录与 kickout
//! 7. GarrisonManager::logic 获取底层 logic 实例
//! 8. 配置访问（GarrisonUtil::config）
//!
//! 运行方式：
//! ```sh
//! cargo run -p garrison-examples --bin manager_lifecycle --features "cache-memory,web-axum"
//! ```
...
// ============================================================================
// 业务方实现 GarrisonInterface
// ============================================================================

/// 示例 GarrisonInterface 实现，提供 login_id → 权限/角色列表。
///
/// 预置数据：
...
pub async fn run() -> GarrisonResult<()> {
    println!("=== Garrison Manager 生命周期示例 ===\n");
    ...
    // ----------------------------------------------------------------
    // 8. GarrisonManager::logic 获取底层实例
    // ----------------------------------------------------------------
    println!("[6] GarrisonManager 底层访问:");
    ...
    // ----------------------------------------------------------------
    // 9. 配置访问（GarrisonUtil::config）
    // ----------------------------------------------------------------
    let config = GarrisonUtil::config()?;
    println!("[7] GarrisonUtil::config():");
