...
//! 上下文与请求示例：演示 GarrisonContext + AxumAdapter 的请求/响应操作。
//!
//! 流程：
//! 1. 构造 axum `Request<Body>`
//! 2. 创建 AxumContext 并获取 GarrisonRequest
...
//! 运行方式：
//! ```sh
//! cargo run -p garrison-examples --bin context_request --features web-axum
//! ```

use axum::body::Body;
use axum::http::Request;
use garrison::config::GarrisonConfig;
use garrison::context::{
    AxumContext, AxumRequest, GarrisonContext, GarrisonRequest, GarrisonResponse,
};
...
pub async fn run() -> GarrisonResult<()> {
    ...
    let req2 = Request::builder()
        .method("POST")
        .uri("/api/data")
        .header("garrison_token", "custom_header_token_456")
        .body(Body::empty())
        .unwrap();
    ...
    println!("    garrison_token: custom_header_token_456");
    ...
    let req3 = Request::builder()
        .method("GET")
        .uri("/")
        .header("Cookie", "garrison_token=cookie_token_789")
        .body(Body::empty())
        .unwrap();
    ...
    println!("    Cookie: garrison_token=cookie_token_789");
    ...
    // 7. 设置自定义 Cookie（dev 模式关闭 Secure）
    // ----------------------------------------------------------------
    let mut dev_config = GarrisonConfig::default_config();
