...
//! 展示：
//! 1. 构建 ApiKeyHandler
//! 2. generate 生成 API Key（64 字符 hex，携带 scopes 与过期时间）
//! 3. verify 校验 Key（返回 ApiKeyInfo 元数据）
//! 4. revoke 吊销 Key
//! 5. rotate 轮换 Key（保留 login_id/scopes/剩余 TTL）
//!
//! 运行方式：
//! ```sh
//! cargo run --example apikey_management --features protocol-apikey
//! ```
...
async fn main() -> BulwarkResult<()> {
    ...
    println!("    新 key verify → login_id={}, scopes={:?}", new_info.login_id, new_info.scopes);
