...
//!
//! 展示：
//! 1. 构建 AuthLogicDefault（注入 BulwarkSession + Token 处理器）
...
//! 运行方式：
//! ```sh
//! cargo run --example auth_logic_impl
//! ```
...
async fn main() -> BulwarkResult<()> {
    ...
    println!("    返回 = {}（校验失败会抛 InvalidToken 错误）", verified_id);
    ...
    assert!(!after_logout);
    println!("\n[8] logout 后 is_login 返回 = {}（会话已销毁）", after_logout);
