...
//!
//! 展示：
//! 1. 实现 BulwarkInterface 提供权限/角色数据
...
//! 运行方式：
//! ```sh
//! cargo run --example permission_check
//! ```
...
async fn main() -> BulwarkResult<()> {
    ...
    println!("    [user:read,  user:delete] → {}（user:read 命中）", any_ok);
    println!("    [user:delete, user:create] → {}（均未命中）\n", any_fail);
    assert!(any_ok);
    assert!(!any_fail);
    ...
    println!("    [user:read, user:delete] → {}（user:delete 缺失）\n", all_fail);
    assert!(all_ok);
    assert!(!all_fail);
