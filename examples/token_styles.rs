...
//!
//! 展示：
//! 1. UUID v4 风格 token（36 字符，无 payload）
//! 2. 64 字符随机 hex 风格 token（无 payload）
//! 3. Simple 风格 token（`<login_id>-<uuid>`，可解析 login_id）
//! 4. TokenStyleFactory 按字符串创建对应风格
//!
//! 运行方式：
//! ```sh
//! cargo run --example token_styles
//! ```
...
fn main() -> BulwarkResult<()> {
    ...
    println!("    parse 返回 TokenClaims {{ login_id: {} }}\n", claims.login_id);
    ...
    #[cfg(feature = "protocol-jwt")]
    {
        ...
        println!("\n    jwt → {}", jwt_token);
        // verify 解析 login_id
        assert_eq!(jwt_handler.verify(&jwt_token)?, Some(3003));
