...
//!
//! 展示：
//! 1. 实现 BulwarkPlugin trait（name / on_login / on_logout / on_permission_check）
//! 2. 通过 inventory::submit! 编译期注册插件
//! 3. BulwarkPluginManager 收集并调用钩子
//! 4. 单个插件失败不中断主流程
//!
//! 运行方式：
//! ```sh
...
impl BulwarkPlugin for AuditPlugin {
    ...
    fn on_login(&self, login_id: i64, token: &str) -> BulwarkResult<()> {
        ...
        println!("    [AuditPlugin] on_login: login_id={}, token={}...",
    ...
    fn on_logout(&self, login_id: i64, token: &str) -> BulwarkResult<()> {
        ...
        println!("    [AuditPlugin] on_logout: login_id={}, token={}...",
            login_id, &token[..8.min(token.len())]);
        Ok(())
    }
    ...
    fn on_permission_check(&self, login_id: i64, permission: &str) -> BulwarkResult<()> {
        ...
        println!("    [AuditPlugin] on_permission_check: login_id={}, permission={}",
            login_id, permission);
        Ok(())
    }
}
...
fn main() -> BulwarkResult<()> {
    ...
    assert!(after > before);
    println!("    AuditPlugin on_login 调用次数 +{}（FailingPlugin 失败未中断）\n", after - before);
    ...
    println!("    on_login            = {}", LOGIN_CALLS.load(Ordering::SeqCst));
    println!("    on_logout           = {}", LOGOUT_CALLS.load(Ordering::SeqCst));
    println!("    on_permission_check = {}", PERM_CHECK_CALLS.load(Ordering::SeqCst));
    ...
    println!("[6] Default::default() 等价于 new()：count={} ✓\n", m2.count());
