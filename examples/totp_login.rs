...
//! 运行方式：
//! ```sh
//! cargo run --example totp_login --features secure-totp
//! ```
//!
//! 本示例演示二步验证流程：
//! 1. 用户已通过密码登录（第一步）
...
#[cfg(not(feature = "secure-totp"))]
fn main() {
    eprintln!("此示例需要启用 secure-totp 特性：");
    eprintln!("  cargo run --example totp_login --features secure-totp");
}
