...
用法：python3 specmark/archive/gen_traceability_matrix.py [--output docs/traceability-matrix.md]
...
归档说明（2026-09）：随 specmark 变更流程收尾自 scripts/ 迁入，
specmark/specs 目录不再入库，脚本保留供历史变更复核使用。
"""
...
# specmark/archive/ → specmark → 项目根（三级）
...
def main() -> int:
    ...
    lines = [
        "# Garrison 需求追溯矩阵（RTM）",
        "",
        "> 本文件由 `specmark/archive/gen_traceability_matrix.py` 自动生成，请勿手改。",
