...
0.3.0 新增 gRPC 鉴权拦截器 `BulwarkGrpcInterceptor`，实现 `tonic::Interceptor`，为 tonic gRPC 服务提供 token 提取与格式校验，通过 `grpc` feature 启用。

## Feature 启用

```toml
[dependencies]
bulwark = { version = "0.7", features = ["grpc"] }
tonic = "0.14"
```
...
> **重要限制**：`tonic::Interceptor::call` 是同步 trait，无法调用异步的 `BulwarkUtil::check_login()`。
> 本拦截器仅完成 token 提取与基本格式校验，**实际的登录态/权限校验**须在 tonic service handler
> 内通过 `BulwarkContext` 显式调用 `BulwarkUtil::check_login()` 完成，或使用 `tower::Layer` middleware。
...
5. 实际登录态/权限校验由业务在 service handler 内通过 `BulwarkContext` 异步完成
...
gRPC 拦截器仅做 token 格式校验，业务方在 service handler 内调用 `BulwarkUtil::check_login` 完成实际鉴权，与 axum / actix-web / warp 共用同一套会话与权限逻辑：
...
- 需在 `BulwarkManager::init` 之后使用，否则 handler 内 `check_login` 返回未初始化错误
