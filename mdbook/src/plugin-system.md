...
pub trait BulwarkPlugin: Send + Sync {
    ...
    /// 登录成功后调用（token 已生成）
    fn on_login(&self, _login_id: &str, _token: &str) -> BulwarkResult<()> { Ok(()) }

    /// 登出时调用（token 即将失效）
    fn on_logout(&self, _login_id: &str, _token: &str) -> BulwarkResult<()> { Ok(()) }

    /// 权限校验时调用（在主校验之后）
    fn on_permission_check(&self, _login_id: &str, _permission: &str) -> BulwarkResult<()> { Ok(()) }
}
```

- trait 为**同步** trait（不使用 `#[async_trait]`），3 个钩子均返回 `BulwarkResult<()>`
...
impl BulwarkPlugin for AuditPlugin {
    ...
    fn on_login(&self, login_id: &str, _token: &str) -> BulwarkResult<()> {
        tracing::info!(login_id, "用户登录审计");
        Ok(())
    }
}

// 工厂函数：返回 Arc<dyn BulwarkPlugin>
fn audit_plugin_factory() -> Arc<dyn BulwarkPlugin> {
    Arc::new(AuditPlugin)
}

// 编译期注册（字段名为 factory，类型 fn() -> Arc<dyn BulwarkPlugin>）
inventory::submit! {
