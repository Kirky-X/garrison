...
pub trait BulwarkDao: Send + Sync {
    ...
    async fn update(&self, key: &str, value: &str) -> BulwarkResult<()>;       // 保留原 TTL
    async fn expire(&self, key: &str, seconds: u64) -> BulwarkResult<()>;
    async fn delete(&self, key: &str) -> BulwarkResult<()>;

    // 扩展方法（含默认实现，后端可重写）
    async fn set_permanent(&self, key: &str, value: &str) -> BulwarkResult<()>;           // 无 TTL 写入
    async fn get_timeout(&self, key: &str) -> BulwarkResult<Option<Duration>>;            // 查询剩余 TTL
    async fn get_with_ttl(&self, key: &str) -> BulwarkResult<Option<(String, Option<Duration>)>>; // 原子 get + TTL
    async fn keys(&self, pattern: &str) -> BulwarkResult<Vec<String>>;                    // glob pattern 扫描
    async fn rename(&self, old_key: &str, new_key: &str) -> BulwarkResult<()>;

    // 原子操作（消除 TOCTOU 竞态，生产部署必须重写为后端原生原子）
    async fn get_and_delete(&self, key: &str) -> BulwarkResult<Option<String>>;           // 一次性消费
    async fn incr(&self, key: &str, ttl_seconds: u64) -> BulwarkResult<u64>;              // 计数器递增
    async fn decr(&self, key: &str) -> BulwarkResult<u64>;                                // 计数器递减
    async fn compare_and_update_if_greater(&self, key: &str, new_value: u64, ttl_seconds: u64) -> BulwarkResult<bool>;

    // SQL 抽象（默认返回 NotImplemented，供未来纯 KV 后端重写）
    async fn find_social_binding(&self, tenant_id: i64, provider: &str, provider_user_id: &str) -> BulwarkResult<Option<i64>>;
    async fn insert_social_binding(&self, tenant_id: i64, login_id: i64, provider: &str, provider_user_id: &str, union_id: Option<&str>, created_at: i64) -> BulwarkResult<()>;
    async fn eval_lua(&self, script: &str, keys: Vec<String>, args: Vec<String>) -> BulwarkResult<Vec<String>>;
}
```

会话层（`BulwarkSession`）基于 `BulwarkDao` 提供 Token-Session 与 Account-Session 双向映射，key 约定：
...
- `keys()` 方法在 `BulwarkDaoOxcache` 上仅在启用 `anomalous-detector-dual` feature 时返回结果（依赖内部 key_index），否则返回 `NotImplemented`
