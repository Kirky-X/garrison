...
- `decode(header_value) -> GarrisonResult<Credential>`：解码 Base64 凭证（不含 `Basic` 前缀）
- `parse_authorization_header(header) -> GarrisonResult<Credential>`：从完整 Authorization header 解析（scheme 大小写不敏感）
- 依赖 `base64` 解码
